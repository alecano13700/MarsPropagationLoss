...
// MarsPropagationLossModel class by Canova Alessandro and Manuele Favero
...
TypeId
MarsPropagationLossModel::GetTypeId (void)
{
    static TypeId tid = TypeId ("ns3::MarsPropagationLossModel")
            .SetParent<PropagationLossModel> ()
            .SetGroupName ("Propagation")
            .AddConstructor<MarsPropagationLossModel> ()
            .AddAttribute ("Exponent",
                           "The exponent of the Path Loss propagation model",
                           DoubleValue (3.0),
                           MakeDoubleAccessor (&MarsPropagationLossModel::m_exponent),
                           MakeDoubleChecker<double> ())
            .AddAttribute ("ReferenceDistance",
                           "The distance at which the reference loss is calculated (m)",
                           DoubleValue (1.0),
                           MakeDoubleAccessor (&MarsPropagationLossModel::m_referenceDistance),
                           MakeDoubleChecker<double> ())
            .AddAttribute ("ReferenceLoss",
                           "The reference loss at reference distance (dB). (Default is Friis at 1m with 5.15 GHz)",
                           DoubleValue (46.6777),
                           MakeDoubleAccessor (&MarsPropagationLossModel::m_referenceLoss),
                           MakeDoubleChecker<double> ())
            .AddAttribute("N_T",
                          "Total particle density",
                          DoubleValue(0),
                          MakeDoubleAccessor (&MarsPropagationLossModel::N_T),
                          MakeDoubleChecker<double> ())
            .AddAttribute("r",
                          "Mean particle radius",
                          DoubleValue(0),
                          MakeDoubleAccessor (&MarsPropagationLossModel::r),
                          MakeDoubleChecker<double> ())
            .AddAttribute("Lambda",
                          "Wavelength",
                          DoubleValue(433),
                          MakeDoubleAccessor (&MarsPropagationLossModel::m_lambda),
                          MakeDoubleChecker<double> ())
            .AddAttribute("EpsilonReal",
                          "Real part of dielettric permittivity of dust particles",
                          DoubleValue(0),
                          MakeDoubleAccessor (&MarsPropagationLossModel::m_epsilon_real),
                          MakeDoubleChecker<double> ())
            .AddAttribute("EpsilonImm",
                          "Imaginary part of dielettric permittivity of dust particles",
                          DoubleValue(0),
                          MakeDoubleAccessor (&MarsPropagationLossModel::m_epsilon_imm),
                          MakeDoubleChecker<double> ())
            .AddAttribute("Distance",
                          "//distance between Tx and Rx",
                          DoubleValue(1),
                          MakeDoubleAccessor (&MarsPropagationLossModel::m_distance),
                          MakeDoubleChecker<double> ())
    ;
    return tid;
}

MarsPropagationLossModel::MarsPropagationLossModel ()
{
}
...
void MarsPropagationLossModel::SetPathLossExponent(double exponent){
    m_exponent = exponent;
}

double MarsPropagationLossModel::GetPathLossExponent(void) const {
    return m_exponent;
}


void MarsPropagationLossModel::SetN_T(double n_t){
    N_T = n_t;
}

double MarsPropagationLossModel::GetN_T(void) const {
    return N_T;
}

void MarsPropagationLossModel::SetParticleRadius(double p_r){
    r = p_r;
}

double MarsPropagationLossModel::GetParticleRadius(void) const {
    return r;
}
void MarsPropagationLossModel::SetLambda(double lambda){
    m_lambda = lambda;
}

double MarsPropagationLossModel::GetLambda(void) const {
    return m_lambda;
}

void MarsPropagationLossModel::SetEpsilonReal(double epsilon){
    m_epsilon_real = epsilon;
}

double MarsPropagationLossModel::GetEpsilonReal(void) const {
    return m_epsilon_real;
}
void MarsPropagationLossModel::SetEpsilonImm(double epsilon){
    m_epsilon_imm = epsilon;
}

double MarsPropagationLossModel::GetEpsilonImm(void) const {
    return m_epsilon_imm;
}

void MarsPropagationLossModel::SetDistance(double distance){
    m_distance = distance;
}

double MarsPropagationLossModel::GetDistance(void) const {
    return m_distance;
}
void MarsPropagationLossModel::SetReference (double referenceDistance, double referenceLoss)
{
  m_referenceDistance = referenceDistance;
  m_referenceLoss = referenceLoss;
}
...
double
MarsPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                         Ptr<MobilityModel> a,
                                         Ptr<MobilityModel> b) const
{
    //rifare
     ...
     */
    double distance = a->GetDistanceFrom (b);
    NS_LOG_DEBUG("TEST_DEBUG: distance:"<<distance);
...
int64_t
MarsPropagationLossModel::DoAssignStreams (int64_t stream)
{
    return 0;
}
